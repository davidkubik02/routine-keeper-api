      ...
      getDocs(collection(db, "users", user, "tasks")).then((response) => {
        ...
        response.forEach((task) => {
          ...
          tasksArray.push({
            ...
            compleated: taskData.compleated,
            compleatedInTime: taskData.compleatedInTime,
          });
        });
...
export const toggleTaskStatus = async (req, res)=>{
  ...
  const compleatedInTime = currenTime <=
      (taskData.deadline === 0 ? Infinity : taskData.deadline)
